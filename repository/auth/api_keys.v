...
pub fn find_by_key(mut pool_conn pool.ConnectionPool, key_value string) !dto.ApiKey {
	conn := pool_conn.get()!
...
pub fn issue(mut pool_conn pool.ConnectionPool, user_id int, label string, plan string) !string {
	conn := pool_conn.get()!
	mut db := conn as pg.DB
	defer {
		pool_conn.put(conn) or { println(err.msg()) }
...
pub fn list_by_user(mut pool_conn pool.ConnectionPool, user_id int) ![]dto.ApiKey {
	conn := pool_conn.get()!
	mut db := conn as pg.DB
	defer {
		pool_conn.put(conn) or { println(err.msg()) }
...
pub fn revoke(mut pool_conn pool.ConnectionPool, user_id int, key_id int) ! {
	conn := pool_conn.get()!
	mut db := conn as pg.DB
	defer {
		pool_conn.put(conn) or { println(err.msg()) }
