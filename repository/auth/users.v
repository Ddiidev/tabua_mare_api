...
pub fn upsert_by_provider(mut db pg.DB, provider string, provider_uid string, email string, name string, avatar_url string, raw_json string) !UpsertResult {
	...
	if rows.len > 0 {
		...
		user_rows := db.exec_param('SELECT email, name, avatar_url, plan FROM users WHERE id = ($1) LIMIT 1',
			user_id.str())!
		if user_rows.len == 0 {
...
pub fn find_by_id(mut db pg.DB, user_id int) !dto.User {
	rows := db.exec_param('SELECT id, email, name, avatar_url, plan, stripe_customer_id, stripe_subscription_id FROM users WHERE id = ($1) LIMIT 1',
		user_id.str())!
	if rows.len == 0 {
		return error('usuario nao encontrado')
	}
